  ...
  return (
    <>
      <section className="home-hero">
        <img src="/images/projects/kitchen-wooley.webp" alt="Completed Versatile Edge kitchen renovation with brick backsplash and large island" />
        ...
        <div className="hero-proof"><div><strong>20+</strong><span>Years of construction experience</span></div><div><strong>NC</strong><span>Licensed general contractor</span></div><div><strong>Local</strong><span>Wake County permitting expertise</span></div></div>
      </section>
      ...
      <section className="feature-project section dark-section"><div className="site-container feature-project-grid"><div className="feature-image"><img src={projects[0].image} alt="Completed brick and brass kitchen renovation" /><span>Actual Versatile Edge project</span></div><div className="feature-copy"><span className="eyebrow light">Featured work</span><h2>A kitchen designed around gathering.</h2><p>Warm brick, tailored lighting, generous work surfaces, and storage with purpose come together in a kitchen that feels both established and completely renewed.</p><dl><div><dt>Project</dt><dd>Full kitchen renovation</dd></div><div><dt>Area</dt><dd>Wake County</dd></div><div><dt>Focus</dt><dd>Flow, storage, finish detail</dd></div></dl><Link href="/projects" className="text-link light">View more projects <ArrowRight size={18} /></Link></div></div></section>
      ...
      <section className="section area-section"><div className="site-container area-grid"><div><span className="eyebrow">Close to home</span><h2>Built for the homes and requirements of Wake County.</h2><p>Local knowledge matters—from permit expectations and inspections to the materials that perform in North Carolina’s climate.</p><div className="area-list">{serviceAreas.map((area) => <span key={area}><MapPin size={15} />{area}</span>)}</div></div><div className="area-image"><img src="/images/projects/three-season-room.webp" alt="Completed three-season room opening to a wooded North Carolina yard" /></div></div></section>
    </>
  );
}
