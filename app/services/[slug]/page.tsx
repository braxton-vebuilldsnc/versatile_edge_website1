...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <PageHero eyebrow={service.slug === "kitchen-renovations" ? "Raleigh & Wake County kitchen remodeling" : service.eyebrow} title={service.title} text={service.slug === "kitchen-renovations" ? "Versatile Edge brings your ideal kitchen to life—with thoughtful layout planning, coordinated selections, and an experienced construction team to carry it through." : service.summary} image={service.image} primaryCta={service.slug === "kitchen-renovations" ? "Request your kitchen consultation" : undefined} secondaryCta={service.slug === "kitchen-renovations" ? { href: phoneHref, label: `Call ${phoneDisplay}` } : undefined} />
    {service.slug === "kitchen-renovations" && <section className="kitchen-landing-intro"><div className="site-container kitchen-landing-intro-inner"><div><span className="eyebrow">Start with a conversation</span><h2>Versatile Edge can create the kitchen you want to come home to.</h2><p>Request a kitchen consultation, and we’ll get back to you to schedule an on-site visit where we can discuss creating your dream kitchen.</p></div><div className="kitchen-landing-actions"><a href="/contact" className="kitchen-primary-cta">Request a kitchen consultation <ArrowRight size={18} /></a><a href={phoneHref} className="kitchen-phone-cta">Prefer to call? <strong>{phoneDisplay}</strong></a></div></div></section>}
