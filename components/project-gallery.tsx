      ...
      <div className="project-grid">
        {visible.map((project, index) => (
          <article className={index % 3 === 0 ? "project-card project-card-wide" : "project-card"} key={project.title}>
            ...
            <div className="project-card-overlay"><span>{project.category} · {project.location}</span><h2>{project.title}</h2><p>{project.summary}</p>{projectPageForTitle(project.title) && <Link className="project-card-link" href={projectPageForTitle(project.title)!}>View project</Link>}</div>
          </article>
        ))}
      </div>
    </>
