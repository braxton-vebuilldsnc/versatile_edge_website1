...
import { projectImageAltText, projectPageForTitle, projects } from "@/lib/site-data";
      ...
      <div className="project-grid">
        {visible.map((project, index) => (
          <article className={index % 3 === 0 ? "project-card project-card-wide" : "project-card"} key={project.title}>
            <ResponsiveImage src={project.image} alt={projectImageAltText[project.image.split("/").at(-1) ?? ""] ?? `${project.title}, a Versatile Edge ${project.category.toLowerCase()} project`} sizes="(max-width: 760px) calc(100vw - 30px), (max-width: 1240px) 50vw, 610px" />
